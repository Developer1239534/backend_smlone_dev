...
async function runForce() {
  try {
    console.log('Resetting portal_admin table without id column...');
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      records.push([
        ...
        cols[13] || null,
        cols[14] || null,
        parseDateStr(cols[15]),
        parseDateStr(cols[16])
      ]);
    }
    ...
    console.log(`Clean single batch insert finished for ${records.length} records!`);

    const finalCount = await db.query(`SELECT COUNT(*) FROM portal_admin;`);
    console.log(`Exact final count in portal_admin: ${finalCount.rows[0].count}`);

  } catch (err) {
    console.error('Error in force import:', err);
  } finally {
    process.exit(0);
  }
}
