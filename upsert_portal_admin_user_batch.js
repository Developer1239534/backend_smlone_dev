...
async function upsertPortalAdminBatch() {
  console.log('🚀 Parsing batch data for portal_admin...');
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    records.push({
      ...
      name,
      level,
      newest_grade: newestGrade,
      house,
      house_role: houseRole,
      trainee_homeroom: traineeHomeroom,
      homeroom_kelas: homeroomKelas,
      trainer,
      membership_status: membershipStatus,
      membership_expired_date: expiryDateParsed,
      first_enroll: firstEnrollParsed,
      expiryDateStr,
      firstEnrollStr
    });
  }

  console.log(`📊 Total records to upsert into portal_admin: ${records.length}`);
  ...
  for (const r of records) {
    const rawDataJson = {
      ...
      name: r.name,
      level: r.level,
      newest_grade: r.newest_grade,
      house: r.house,
      house_role: r.house_role,
      trainee_homeroom: r.trainee_homeroom,
      homeroom_kelas: r.homeroom_kelas,
      trainer: r.trainer,
      membership_status: r.membership_status,
      membership_expired_date: r.expiryDateStr,
      first_enroll: r.firstEnrollStr
    };

    // Upsert into portal_admin (where trainee_id matches)
    const query = `
    ...
    const values = [
      ...
      r.house || null,
      r.house_role || null,
      r.trainee_homeroom || null,
      r.homeroom_kelas || null,
      r.trainer || null,
      r.membership_status || null,
      r.membership_expired_date,
      r.first_enroll,
    ...
    try {
      ...
      // If unique constraint on trainee_id doesn't exist yet on portal_admin, do a fallback update/insert
      if (e.code === '42883' || e.message.includes('ON CONFLICT')) {
        const updateRes = await db.query(
          `UPDATE portal_admin SET
            class_name = $1, day = $2, time = $3, room = $4, branch = $5, name = $6,
            level = $7, newest_grade = $8, house = $9, house_role = $10,
            trainee_homeroom = $11, homeroom_kelas = $12, trainer = $13,
            membership_status = $14, membership_expired_date = $15, first_enroll = $16,
            raw_data = $17, updated_at = NOW()
           WHERE trainee_id = $18 RETURNING *`,
          [
            r.class_name || null, r.day || null, r.time || null, r.room || null, r.branch || null,
            r.name, r.level || null, r.newest_grade || null, r.house || null, r.house_role || null,
            r.trainee_homeroom || null, r.homeroom_kelas || null, r.trainer || null,
            r.membership_status || null, r.membership_expired_date, r.first_enroll,
            JSON.stringify(rawDataJson), r.trainee_id
          ]
        );
        ...
        console.error(`Error processing trainee_id ${r.trainee_id}:`, e.message);
      }
    }
  }

  console.log(`\n🎉 SUCCESS! Successfully upserted ${upsertedCount} records into portal_admin.`);

  // Verify total count in portal_admin
  const totalRes = await db.query(`SELECT COUNT(*) FROM portal_admin`);
  console.log(`📌 Total records in portal_admin table: ${totalRes.rows[0].count}`);

  // Sample check
  const sampleRes = await db.query(`SELECT trainee_id, name, class_name, branch, membership_status, raw_data FROM portal_admin WHERE trainee_id IN ('980', '1176', '70100104', '670')`);
  console.log('\n🔍 Sample updated portal_admin records:', sampleRes.rows);
}

upsertPortalAdminBatch().catch(console.error).then(() => process.exit(0));
