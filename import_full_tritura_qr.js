...
async function main() {
  ...
  console.log('📖 Reading full transcript from:', fullTranscriptPath);
  ...
  for (let line of lines) {
    ...
    // Pattern 1: Tab or space separated: ID Title URL
    // e.g. "96  Jan - Mar 2023  https://docs.google.com/..."
    const matchLine = line.match(/^([a-zA-Z0-9]+)\s+.*?\s+(https?:\/\/[^\s]+)/);
    if (matchLine) {
      const id = matchLine[1].trim();
      const url = matchLine[2].trim();
    ...
    if (line.startsWith('http://') || line.startsWith('https://')) {
      if (pendingId && pendingId.toLowerCase() !== 'yang') {
        items.set(pendingId, line.trim());
        pendingId = null;
      }
      continue;
    }

    // Pattern 3: Standalone ID line
  ...
  console.log(`🚀 Total UNIQUE trainee IDs parsed from full text: ${items.size}`);
