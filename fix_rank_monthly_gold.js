...
function parseExactRanks(input) {
  ...
  for (const line of lines) {
    ...
    while (idx < parts.length) {
      if (/^\d+$/.test(parts[idx])) {
        ...
        // Find candidate rank number following juniorYouth
        let rankId = '';
        for (let k = idx + 9; k < Math.min(idx + 14, parts.length); k++) {
          const val = parts[k] ? parts[k].trim() : '';
          if (/^\d+$/.test(val)) {
            rankId = val;
            break;
          }
...
async function runExactRankSeed() {
  ...
  try {
    ...
    const records = parseExactRanks(rawInput);
    console.log(`📌 Found ${records.length} unique valid trainee records with ranks.`);
    ...
    console.log(`✅ Success! Seeded ${count} records into "monthly_gold_point".`);

    const totalRes = await client.query('SELECT COUNT(*) FROM monthly_gold_point;');
    console.log(`📊 Total rows in "monthly_gold_point": ${totalRes.rows[0].count}`);
