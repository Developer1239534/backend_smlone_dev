...
async function dedupeAndUpsertPortalAdmin() {
  console.log('🧹 Cleaning up duplicate trainee_id rows in portal_admin...');
  
  // Clean duplicates in DB keeping min id
  await db.query(`
    DELETE FROM portal_admin p1
    USING portal_admin p2
    WHERE p1.ctid > p2.ctid AND p1.trainee_id = p2.trainee_id;
  `);

  console.log('🔒 Adding UNIQUE constraint on trainee_id...');
  try {
    await db.query(`
      ALTER TABLE portal_admin DROP CONSTRAINT IF EXISTS portal_admin_trainee_id_key;
      ALTER TABLE portal_admin ADD CONSTRAINT portal_admin_trainee_id_key UNIQUE (trainee_id);
    `);
  ...
  console.log(`📊 Fast multi-row upserting ${entries.length} trainees into portal_admin...`);
  ...
  // Also synchronize any new trainee into tabel_login_trainee if missing
  console.log('🔄 Syncing newly inserted trainees to `tabel_login_trainee`...');
  await db.query(`
    INSERT INTO tabel_login_trainee (trainee_id, nama, password, plain_password, created_at, updated_at)
    SELECT 
      pa.trainee_id,
      pa.name AS nama,
      'SML' || pa.trainee_id AS password,
      'SML' || pa.trainee_id AS plain_password,
      NOW(), NOW()
  ...
  console.log('\n🎉 SUCCESS! Batch Upsert + Sync completed without errors.');
  ...
  const sampleRes = await db.query(`SELECT trainee_id, name, class_name, branch, membership_status, membership_expired_date, first_enroll FROM portal_admin WHERE trainee_id IN ('980', '1176', '70100104', '670', '1121')`);
  console.log('\n🔍 Sample records in database:', sampleRes.rows);
}

dedupeAndUpsertPortalAdmin().catch(console.error).then(() => process.exit(0));
