...
async function main() {
  ...
  console.log('📖 Reading full transcript for Cemara Referral Code import...');
  ...
  console.log(`🚀 Total UNIQUE trainee IDs parsed for Cemara Referral Code: ${items.size}`);
  ...
  const totalCemaraRef = await db.query("SELECT COUNT(*) FROM portal_trainee WHERE branch_id = 'cemara' AND referral_code IS NOT NULL;");
  ...
  console.log(`📈 Total portal_trainee 'cemara' with referral_code: ${totalCemaraRef.rows[0].count}`);
}
