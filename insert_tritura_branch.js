...
async function main() {
  ...
  console.log('📖 Reading full transcript for Tritura branch IDs...');
  ...
  for (let line of lines) {
    ...
    if (/^[a-zA-Z0-9]+$/.test(line) && line !== 'ID' && line.toLowerCase() !== 'ini' && line.toLowerCase() !== 'adalah' && line.toLowerCase() !== 'untuk' && line.toLowerCase() !== 'di' && line.toLowerCase() !== 'cabang' && line.toLowerCase() !== 'tritura' && line.toLowerCase() !== 'ya' && line.toLowerCase() !== 'letakkan' && line.toLowerCase() !== 'dan' && line.toLowerCase() !== 'isi' && line.toLowerCase() !== 'coloum' && line.toLowerCase() !== 'dengan') {
      trituraIds.add(line);
    }
  }

  console.log(`🚀 Total UNIQUE trainee IDs parsed for Tritura branch: ${trituraIds.size}`);
  ...
  const totalTritura = await db.query("SELECT COUNT(*) FROM portal_trainee WHERE branch_id = 'tritura';");
  ...
  console.log(`📈 Total portal_trainee branch 'tritura': ${totalTritura.rows[0].count}`);
}
