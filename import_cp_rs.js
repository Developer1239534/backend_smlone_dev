...
async function main() {
  ...
  console.log('📖 Reading full transcript for CP Real Stage Report import...');
  ...
  const items = new Map(); // id -> latest google drive url
  ...
  for (let line of lines) {
    ...
    if (line.startsWith('http://') || line.startsWith('https://') || line.includes('drive.google.com')) {
      ...
      if (pendingId && pendingId !== 'ID' && pendingId !== 'Report' && pendingId !== 'Title' && pendingId !== 'Link' && pendingId.toLowerCase() !== 'yang' && !pendingId.toLowerCase().startsWith('real stage')) {
    ...
    // Pattern 3: Standalone ID line (alphanumeric, avoiding headers/real stage labels)
  ...
  console.log(`🚀 Total UNIQUE trainee IDs parsed for CP Real Stage Report: ${items.size}`);
  ...
  const totalCPRS = await db.query("SELECT COUNT(*) FROM portal_trainee WHERE branch_id = 'cp' AND real_stage_report_url IS NOT NULL;");
  ...
  console.log(`📈 Total portal_trainee 'cp' with real_stage_report_url: ${totalCPRS.rows[0].count}`);
}
