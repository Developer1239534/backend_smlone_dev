...
async function main() {
  ...
  console.log('📖 Reading full transcript for Referral Code import...');
  ...
  for (let line of lines) {
    ...
    // Pattern 1: ID  REFERRAL CODE  CODE (tab or space separated)
    // e.g. "70100004  REFERRAL CODE  714MARYAM" or "35  REFERRAL CODE  ANAAS001"
    const match = line.match(/^([a-zA-Z0-9]+)\s+REFERRAL\s*CODE\s+([a-zA-Z0-9_\-]+)/i);
    if (match) {
  ...
  console.log(`🚀 Total UNIQUE trainee IDs parsed for Referral Code: ${items.size}`);
  ...
  const totalRef = await db.query("SELECT COUNT(*) FROM portal_trainee WHERE referral_code IS NOT NULL;");
  ...
  console.log(`📈 Total portal_trainee with referral_code: ${totalRef.rows[0].count}`);
}
