...
const rawTsvData = `CLASS	Day	Time	Room	Branch	ID	Name	Level	NEWEST GRADE	HOUSE	House Role	Trainee Homeroom	Homeroom kelas	Trainer	MEMBERSHIP	EXPIRY DATE	FIRST ENROLL
...
Clinton (Fri 3-5)	Friday	15:00-17:00	Schomberg	Centre Point	654	Rayden Oh	Lt. Colonel	8	House of Thenova		Muly	Muly	Ricky	Expired	2 Jul 2026	12 Jul 2023
...
Atlanta	Friday	15:30-17:30	Kirana	Tritura	70100046	Kirania Inara Azalea	Lt. Colonel	7	House of Thenova		Loita	Loita	Devina, Angeline	Active	18 Sep 2026	18 Jul 2024
...
Gates (Sat 10-12)	Saturday	10:00-12:00	Schomberg	Centre Point	822	Clarissa Olivia Anne Lammora Panjaitan	Colonel	9	House of Havaria		Ghaitsa	Ghaitsa	Ghaitsa	Active	2 Sep 2026	30 Apr 2024
...
Millman (Sat 1-3)	Saturday	13:00-15:00	Clapham	Centre Point	48	Justin Maxwell	General	9	House of Havaria		Ghaitsa	Ghaitsa	Ricky	Active	1 Sep 2026	02 Nov 2019
...
Doyle (Sat 1-3)	Saturday	13:00-15:00	Schomberg	Centre Point	1049	Rafael Maximillian Sitorus	Sergeant	10	House of Havaria		Rizky	Ghaitsa	Denny	Active (Grace Period)	2 Aug 2026	30 Jul 2025
Doyle (Sat 1-3)	Saturday	13:00-15:00	Schomberg	Centre Point	1050	Galang Roland Besch	Sergeant	10	House of Havaria		Rizky	Ghaitsa	Denny	Active (Grace Period)	2 Aug 2026	30 Jul 2025
...
Amber	Saturday	13:00-15:00	Lyra	Cemara	90100047	Bryant Maximus Ling	Sergeant	4	House of Thenova		Loita	Nabila	Mulyanita	Active	1 Sep 2026	01 Feb 2025
...
Pearl	Saturday	16:00-18:00	Orion	Cemara	90100005	Felynn Holy Richson	Lt. Colonel	4	House of Thenova		Agustina	Agustina	Loita	Expired	25 Jul 2026	25 Jan 2025
...
Mandela	Saturday	16:00-18:00	Chesterfield	Centre Point	962	Ananda Putera Ngadiman	Sergeant	5	House of Havaria		Rizky	Rizky	Rizky	Active	1 Sep 2026	08 Feb 2025
...
async function runImport() {
  try {
    console.log('🏗️ Creating table portal_trainee if not exists...');
    await db.query(`
    ...
    console.log('🧹 Truncating portal_trainee table before import...');
    await db.query('TRUNCATE TABLE portal_trainee CASCADE;');

    const lines = rawTsvData.trim().split('\n');
    console.log(`Total lines in raw dataset: ${lines.length - 1}`);

    // Map to deduplicate trainee_id (keeping the latest valid record)
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      const className = cols[0] || null;
      const day = cols[1] || null;
      const time = cols[2] || null;
      const room = cols[3] || null;
      const branch = cols[4] || null;
      const traineeId = cols[5] || null;
      const name = cols[6] || null;
      const level = cols[7] || null;
      const newestGrade = cols[8] || null;
      const house = cols[9] || null;
      const houseRole = cols[10] || null;
      const traineeHomeroom = cols[11] || null;
      const homeroomKelas = cols[12] || null;
      const trainer = cols[13] || null;
      const membershipStatus = cols[14] || null;
      const expiryDate = parseDate(cols[15]);
      const firstEnroll = parseDate(cols[16]);

      if (!traineeId) continue;

      traineeMap.set(traineeId, {
        traineeId, name, className, day, time, room, branch, level,
        newestGrade, house, houseRole, traineeHomeroom, homeroomKelas,
        trainer, membershipStatus, expiryDate, firstEnroll
      });
    }

    console.log(`Deduplicated unique trainees count: ${traineeMap.size}`);

    const valueRows = [];
    const params = [];
    let paramIdx = 1;

    for (const record of traineeMap.values()) {
      const rowPlaceholders = [];
      const rowData = [
        record.traineeId, record.name, record.className, record.day, record.time,
        record.room, record.branch, record.level, record.newestGrade, record.house,
        record.houseRole, record.traineeHomeroom, record.homeroomKelas, record.trainer,
        record.membershipStatus, record.expiryDate, record.firstEnroll
      ];
      ...
      valueRows.push(`(${rowPlaceholders.join(', ')}, NOW())`);
    }

    if (valueRows.length > 0) {
      const queryStr = `
        INSERT INTO portal_trainee (
          trainee_id, name, class, day, time, room, branch_id, level,
          newest_grade, house, house_role, trainee_homeroom, homeroom_kelas,
          trainer, membership_status, membership_expired_date, first_enroll, updated_at
    ...
    console.log(`\n🎉 FAST DEDUPLICATED BATCH IMPORT COMPLETED! Successfully inserted ${traineeMap.size} trainees.`);

    const countRes = await db.query('SELECT COUNT(*) FROM portal_trainee;');
    console.log(`Current total row count in portal_trainee: ${countRes.rows[0].count}`);

    const sampleRes = await db.query('SELECT trainee_id, name, class, branch_id, membership_status, membership_expired_date, first_enroll FROM portal_trainee LIMIT 5;');
    console.log('Sample inserted rows:', JSON.stringify(sampleRes.rows, null, 2));
