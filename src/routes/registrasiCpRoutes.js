...
// Helper to map DB row to Cp layout
const mapRowToCp = (row) => {
  return {
    id: row.id,
    ...
    raw_data: {
      ...
      House: row.house,
      'CABANG KELAS': row.cabang_kelas || 'CP',
...
router.get('/', async (req, res) => {
  try {
    ...
    res.json({
      ...
      data: savedData.rows.map(mapRowToCp)
    });
  } catch (error) {
...
router.get('/:id', async (req, res) => {
  try {
    ...
    res.json({ success: true, data: mapRowToCp(result.rows[0]) });
  } catch (error) {
...
async function upsertTrainee(id, updates, cabangId = 'TIMOR') {
  const columns = [];
  const placeholders = [];
  const values = [];
  let index = 1;
...
router.post('/', async (req, res) => {
  try {
    ...
    const result = await upsertTrainee(id, rawData, 'TIMOR');
    res.status(201).json({ success: true, message: 'Berhasil menyimpan data registrasi CP.', data: mapRowToCp(result) });
  } catch (error) {
...
router.put('/:id', async (req, res) => {
  ...
  try {
    const result = await upsertTrainee(id, updates, 'TIMOR');
    res.json({ success: true, message: 'Data berhasil diupdate.', data: mapRowToCp(result) });
  } catch (error) {
