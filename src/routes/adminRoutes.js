...
// 12. PATCH /admin/students/:id/gold-points - Update Total Gold Points
// Body: { gold_points }
router.patch('/students/:id/gold-points', async (req, res) => {
  const { id } = req.params;
  const { gold_points } = req.body;

  if (gold_points === undefined) {
    return res.status(400).json({ success: false, message: 'gold_points wajib diisi.' });
  }
  ...
  try {
    ...
    await db.query(
      `UPDATE dashboard_trainne SET gold_points = $1 WHERE id = $2`,
      [gold_points, id]
    );

    res.json({ success: true, message: 'Total Gold Points berhasil diperbarui.', data: { id, gold_points } });
  } catch (err) {
    console.error('[Admin] Update Gold Points error:', err.message);
    res.status(500).json({ success: false, message: 'Server error.' });
  }
});

// ========================================================
// NEWS & ANNOUNCEMENTS ENDPOINTS (For Portal)
// ========================================================
