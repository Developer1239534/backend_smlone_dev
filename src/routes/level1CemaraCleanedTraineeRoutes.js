...
router.get('/', async (req, res) => {
  try {
    const savedData = await db.query('SELECT * FROM level_1_ca_cleaned_trainee ORDER BY created_at DESC');
    ...
    const mappedData = savedData.rows.map(row => ({
      ...
      branch: row.cabang,
      membership: row.membership_status || row.clean_membership_status
    }));

    res.json({
      success: true,
      message: 'Berhasil mengambil data cleaned trainee.',
      data: mappedData
    });
  } catch (error) {
    console.error('Error fetching cleaned trainees:', error.message);
    res.status(500).json({ 
      success: false, 
...
router.post('/push', async (req, res) => {
  try {
    ...
    console.log(`[n8n Push CA Cleaned] Received ${data.length} items`);

    for (let i = 0; i < data.length; i++) {
      const row = data[i];

      if (!row || typeof row !== 'object') {
        console.warn(`[n8n Push CA Cleaned] Skipping invalid row at index ${i}:`, row);
        skippedCount++;
        continue;
      }
      ...
      if (!email_account || !first_name) {
        console.warn(`[n8n Push CA Cleaned] Skipping row ${i}: missing email_account="${email_account}" or first_name="${first_name}". Keys: ${Object.keys(row).join(', ')}`);
        skippedCount++;
        continue;
      }
      ...
      const less_than_2022 = row['Less Than 2022 Students Grade Helper'] || row['less_than_2022_students_grade_helper'] || '';

      const raw_data = JSON.stringify(row);

      try {
        const upsertQuery = `
          INSERT INTO level_1_ca_cleaned_trainee (
            ...
            email_trainee, check_double_id, new_profile_picture, less_than_2022_students_grade_helper, raw_data
          ) VALUES (
          ...
          DO UPDATE SET
            ...
            new_parent_email = EXCLUDED.new_parent_email,
        ...
        await db.query(upsertQuery, [
          ...
          class_status, cleaned_program, membership_status, clean_membership_status, check_ac_ad, cabang, 
          clean_parents_email, new_parent_email, class_name, house, level, house_role, nomor_trainee, 
          email_trainee, check_double_id, new_profile_picture, less_than_2022, raw_data
        ]);
        insertedCount++;
      } catch (rowError) {
        errorCount++;
        errors.push({ index: i, email: email_account, name: first_name, error: rowError.message });
        console.error(`[n8n Push CA Cleaned] Error on row ${i} (${first_name} / ${email_account}):`, rowError.message);
      }
    }

    console.log(`[n8n Push CA Cleaned] Done: inserted=${insertedCount}, skipped=${skippedCount}, errors=${errorCount}`);
    ...
    console.error('[n8n Push CA Cleaned] Fatal error:', error.message);
    res.status(500).json({ 
      success: false, 
...
router.put('/:id', async (req, res) => {
  ...
  try {
    ...
    values.push(id);
    const query = `UPDATE level_1_ca_cleaned_trainee SET ${setQuery.join(', ')} WHERE id = $${index} RETURNING *`;
...
router.delete('/:id', async (req, res) => {
  ...
  try {
    const result = await db.query('DELETE FROM level_1_ca_cleaned_trainee WHERE id = $1 RETURNING id', [id]);
