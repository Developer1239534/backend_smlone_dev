...
(async () => {
  try {
    ...
    // Auto-seed all 78 feedback records on server startup
    try {
      const feedbackSeed = require('./routes/seed_feedback_78.json');
      if (Array.isArray(feedbackSeed) && feedbackSeed.length > 0) {
        ...
        console.log(`✅ Auto-seeded ${feedbackSeed.length} feedback records on server startup!`);
      }
    } catch (seedErr) {
      console.error('Error auto-seeding feedback table on startup:', seedErr.message);
    }
