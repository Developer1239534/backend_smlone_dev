...
async function main() {
  ...
  console.log('📖 Reading full transcript for Cemara Quarterly Report import...');
  ...
  for (let line of lines) {
    ...
    // Markdown link pattern: [https://docs.google.com/...](https://docs.google.com/...)
    const mdMatch = line.match(/\]\((https?:\/\/[^\s)]+)\)/);
    if (mdMatch || line.startsWith('http://') || line.startsWith('https://')) {
      const url = mdMatch ? mdMatch[1] : line;
    ...
    // Standalone ID (numbers/alphanumeric)
  ...
  console.log(`🚀 Total UNIQUE trainee IDs parsed for Cemara Quarterly Report: ${items.size}`);
  ...
  const totalCemaraQR = await db.query("SELECT COUNT(*) FROM portal_trainee WHERE branch_id = 'cemara' AND quarterly_report_url IS NOT NULL;");
  ...
  console.log(`📈 Total portal_trainee 'cemara' with quarterly_report_url: ${totalCemaraQR.rows[0].count}`);
}
