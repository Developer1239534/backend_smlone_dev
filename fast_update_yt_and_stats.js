...
async function fastBatchUpdate() {
  try {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (/^\d+$/.test(cleanedLine)) {
        if (currentRecord.id) {
          map.set(currentRecord.id, currentRecord);
        }
    ...
    console.log(`Parsed ${records.length} unique trainee records.`);
    ...
    for (let c = 0; c < records.length; c += chunkSize) {
      ...
      chunk.forEach(r => {
        ...
        queryParams.push(r.id, defaultTerm, r.nama || null, r.status || null, r.link_youtube || null);
        paramIdx += 5;
      });

      const batchQuery = `
        INSERT INTO link_report (trainee_id, term, nama, status, link_youtube)
    ...
    console.error('Error in fast batch update:', err);
    process.exit(1);
  }
}
