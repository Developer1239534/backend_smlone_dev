...
async function main() {
  ...
  console.log('📖 Reading full transcript for Cemara Progress Video import...');
  ...
  for (let line of lines) {
    ...
    // Pattern 1: Same line "602  ▶️ Progress Video  https://..."
    const matchSameLine = line.match(/^([a-zA-Z0-9]+)\s+▶️\s*Progress\s*Video\s+(https?:\/\/[^\s]+)/i);
    if (matchSameLine) {
    ...
    if (line.startsWith('http://') || line.startsWith('https://') || line.includes('youtube.com')) {
      let url = line;
  ...
  console.log(`🚀 Total UNIQUE trainee IDs parsed for Cemara Progress Video: ${items.size}`);
