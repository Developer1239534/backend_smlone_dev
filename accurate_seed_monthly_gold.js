  ...
  for (const line of lines) {
    ...
    while (idx < parts.length) {
      ...
      if (/^\d+$/.test(parts[idx])) {
        ...
        const rankId = parts[idx + 9] ? parts[idx + 9].trim() : '';

        // Validate basic trainee structure
...
async function seedDataAccurately() {
  ...
  try {
    ...
    const records = parseAllRecords(rawInput);
    console.log(`📌 Found ${records.length} unique valid trainee records.`);
    ...
    console.log(`✅ Seed selesai! Berhasil menyimpan ${count} data trainee ke "monthly_gold_point".`);

    const totalRes = await client.query('SELECT COUNT(*) FROM monthly_gold_point;');
    console.log(`📊 Total baris bersih di tabel "monthly_gold_point": ${totalRes.rows[0].count}`);

    const sampleRes = await client.query('SELECT * FROM monthly_gold_point ORDER BY "ID" LIMIT 5;');
    console.log('\n🔍 Sample Data Rapi di NeonDB:');
