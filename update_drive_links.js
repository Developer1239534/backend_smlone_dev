...
async function updateDriveLinks() {
  try {
    const lines = rawDriveInput.trim().split('\n').map(l => l.trim()).filter(l => l.length > 0);
    const map = new Map();
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (/^\d+$/.test(cleaned)) {
        currentId = cleaned;
      } else if (cleaned.toLowerCase().includes('202') || cleaned.toLowerCase().includes('may')) {
        currentTerm = cleaned;
      } else if (cleaned.startsWith('http://') || cleaned.startsWith('https://')) {
        if (currentId) {
          map.set(currentId, {
            id: currentId,
            term: currentTerm || 'May 2026 - Jun 2026',
            link_term: cleaned
          });
        }
      }
    ...
    console.log(`Parsed ${records.length} unique drive report records.`);
    ...
    for (let c = 0; c < records.length; c += chunkSize) {
      ...
      chunk.forEach(r => {
        valueStrings.push(`($${paramIdx}, $${paramIdx+1}, $${paramIdx+2})`);
        queryParams.push(r.id, r.term, r.link_term);
        paramIdx += 3;
      });
    ...
    console.log('⚡ Drive report links updated successfully!');
    ...
    console.log('\n=========================================');
    console.log(`📊 FINAL COUNT: Active + Grace Period with BOTH links: ${totalBoth.rows[0].count}`);
    ...
    console.error('Error updating drive links:', err);
    process.exit(1);
  }
}
