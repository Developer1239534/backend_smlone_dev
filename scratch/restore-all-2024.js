...
const files = [
  path.join(__dirname, 'full-user-prompt-2024.txt'),
  path.join(__dirname, 'additional-user-prompt-2024.txt')
];
...
async function main() {
  try {
    ...
    for (const filePath of files) {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        if (
          ...
          line.startsWith('</USER_REQUEST>') || 
          line.startsWith('Ini periode 2024 ya') ||
          line.startsWith('QUARTERLY REPORT') ||
          line.startsWith('ID\tPeriode') ||
        ...
        // Skip truncated URLs (length < 60)
        if (!url || url.length < 60) {
          truncatedUrls.push({ file: path.basename(filePath), lineNum: i + 1, id, url });
          continue;
        }

        const dedupKey = `${id}|||${periode}`;
        if (dedupSet.has(dedupKey)) {
          duplicates.push({ file: path.basename(filePath), lineNum: i + 1, id, url });
          continue;
        }
    ...
    console.log(`\n===========================================`);
    console.log(`   FULL 2024 RESTORE COMPLETED`);
    console.log(`===========================================`);
