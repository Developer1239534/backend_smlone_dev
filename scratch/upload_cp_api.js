...
conn.on('ready', () => {
  ...
  conn.sftp((err, sftp) => {
    ...
    sftp.fastPut('./src/server.js', '/root/backend_smlone_dev/src/server.js', (err) => {
      ...
      sftp.fastPut('./src/routes/level1CpRegistrationsRoutes.js', '/root/backend_smlone_dev/src/routes/level1CpRegistrationsRoutes.js', (err) => {
        if (err) throw err;
        console.log('Uploaded level1CpRegistrationsRoutes.js');
