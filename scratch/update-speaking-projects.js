...
async function main() {
  try {
    ...
    for (const line of lines) {
      ...
      dedupSet.add(id);
      records.push({ id, project: project || null });
    }

    console.log(`Parsed ${records.length} unique trainees for project updates.`);
    ...
    for (const record of records) {
      ...
      await db.query('UPDATE dashboard_trainne SET last_speaking_project = $1 WHERE id = $2', [record.project, targetId]);
      updatedCount++;
    }

    console.log(`\n✅ Success! Updated ${updatedCount} speaking projects in the database.`);
    if (missingIds.length > 0) {
      console.warn(`⚠️ Warning: The following ${missingIds.length} IDs were not found in the database (not updated):`, missingIds);
    }
