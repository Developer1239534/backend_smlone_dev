...
const dataFilePath = path.join(__dirname, 'full-user-prompt-2024.txt');
...
async function main() {
  try {
    ...
    for (const line of lines) {
      ...
      // If URL is missing or placeholder/truncated, store it as empty string
      let url = parts[2] ? parts[2].trim() : '';

      // Skip headers or invalid lines (id must be a number)
      if (!id || isNaN(id) || !periode) continue;

      // Skip the truncation notice line if it somehow got parsed as a number
    ...
    console.log(`Parsed ${records.length} unique quarterly report records for 2024.`);
    ...
    console.log(`\n=======================================`);
    console.log(`    2024 REPORT SEEDING COMPLETED      `);
    console.log(`=======================================`);
