...
async function main() {
  try {
    ...
    console.log(`Total 2026 records parsed:     ${records.length}`);
    ...
    for (const rec of records) {
      ...
      if (reportCheck.rows.length > 0) {
        ...
        newRecords.push({ id: rec.id, name: traineeName, url: rec.url });
      }
    }
    ...
    if (newRecords.length > 0) {
      console.log('New Records:', newRecords.map(r => `${r.id} (${r.name}): ${r.url}`));
    }
