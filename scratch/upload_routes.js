...
conn.on('ready', () => {
  ...
  conn.sftp((err, sftp) => {
    ...
    const file2Local = './src/routes/adminRegistrationsRoutes.js';
    const file2Remote = '/root/backend_smlone_dev/src/routes/adminRegistrationsRoutes.js';
    ...
    sftp.fastPut(file1Local, file1Remote, (err) => {
      ...
      sftp.fastPut(file2Local, file2Remote, (err) => {
        if (err) throw err;
        console.log('Updated adminRegistrationsRoutes.js');
