...
const rawData = `CLASS	Day	Time	Room	Branch	ID	Name	Level	NEWEST GRADE	HOUSE	House Role	Trainee Homeroom	Homeroom kelas	Trainer	MEMBERSHIP	EXPIRY DATE	FIRST ENROLL
...
async function run() {
  try {
    console.log('Creating portal_admin table if not exists...');
    await db.query(`DROP TABLE IF EXISTS portal_admin;`);
    await db.query(`
    ...
    `);
    console.log('Table portal_admin created.');
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      const class_name = cols[0] || null;
      const day = cols[1] || null;
      const time = cols[2] || null;
      const room = cols[3] || null;
      const branch = cols[4] || null;
      const trainee_id = cols[5] || null;
      const name = cols[6] || null;
      const level = cols[7] || null;
      const newest_grade = cols[8] || null;
      const house = cols[9] || null;
      const house_role = cols[10] || null;
      const trainee_homeroom = cols[11] || null;
      const homeroom_kelas = cols[12] || null;
      const trainer = cols[13] || null;
      const membership_status = cols[14] || null;
      const expiry_date = parseDateStr(cols[15]);
      const first_enroll = parseDateStr(cols[16]);
      ...
      `, [
        class_name, day, time, room, branch, trainee_id, name, level,
        newest_grade, house, house_role, trainee_homeroom, homeroom_kelas,
        trainer, membership_status, expiry_date, first_enroll
      ]);
      inserted++;
    }

    console.log(`Successfully inserted ${inserted} records into portal_admin!`);

    const countRes = await db.query(`SELECT COUNT(*) FROM portal_admin;`);
    console.log(`Total rows in portal_admin: ${countRes.rows[0].count}`);

  } catch (err) {
    console.error('Error populating portal_admin:', err);
  } finally {
    process.exit(0);
  }
}
